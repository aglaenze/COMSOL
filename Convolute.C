...
int Convolute(int modelNum, string gasName, vector<int> hvList, bool ibf = true) {
    ...
    if (!ibf) fSignalName += "-noibf";
    fSignalName += fInputHV;
    TString fOutputName = path+ "fe-spectrum-convoluted";
    if (!ibf) fOutputName += "-noibf";
    fOutputName += fInputHV;
